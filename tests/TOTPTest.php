...
final class TOTPTest extends TestCase
{
    ...
    public function dataVectors()
    {
        ...
        return [[$totp_sha1, 59, '94287082'], [$totp_sha256, 59, '46119246'], [$totp_sha512, 59, '90693936'], [$totp_sha1, 1111111109, '07081804'], [$totp_sha256, 1111111109, '68084774'], [$totp_sha512, 1111111109, '25091201'], [$totp_sha1, 1111111111, '14050471'], [$totp_sha256, 1111111111, '67062674'], [$totp_sha512, 1111111111, '99943326'], [$totp_sha1, 1234567890, '89005924'], [$totp_sha256, 1234567890, '91819424'], [$totp_sha512, 1234567890, '93441116'], [$totp_sha1, 2000000000, '69279037'], [$totp_sha256, 2000000000, '90698825'], [$totp_sha512, 2000000000, '38618901'], [$totp_sha1, 20000000000, '65353130'], [$totp_sha256, 20000000000, '77737706'], [$totp_sha512, 20000000000, '47863826']];
    }
    public function testVerifyOtpInWindow()
    {
        $otp = $this->createTOTP(6, 'sha1', 30);
        $this->assertFalse($otp->verify('054409', 319690800, 10));
        // -11 periods
        $this->assertTrue($otp->verify('808167', 319690800, 10));
        // -10 periods
        $this->assertTrue($otp->verify('364393', 319690800, 10));
        // -9 periods
        $this->assertTrue($otp->verify('762124', 319690800, 10));
        // 0 periods
        $this->assertTrue($otp->verify('988451', 319690800, 10));
        // +9 periods
        $this->assertTrue($otp->verify('789387', 319690800, 10));
        // +10 periods
        $this->assertFalse($otp->verify('465009', 319690800, 10));
        // +11 periods
    }
    public function testVerifyOtpWithEpochInWindow()
    {
        $otp = $this->createTOTP(6, 'sha1', 30, 'JDDK4U6G3BJLEZ7Y', 'alice@foo.bar', 'My Project', 100);
        $this->assertFalse($otp->verify('054409', 319690900, 10));
        // -11 periods
        $this->assertTrue($otp->verify('808167', 319690900, 10));
        // -10 periods
        $this->assertTrue($otp->verify('364393', 319690900, 10));
        // -9 periods
        $this->assertTrue($otp->verify('762124', 319690900, 10));
        // 0 periods
        $this->assertTrue($otp->verify('988451', 319690900, 10));
        // +9 periods
        $this->assertTrue($otp->verify('789387', 319690900, 10));
        // +10 periods
        $this->assertFalse($otp->verify('465009', 319690900, 10));
        // +11 periods
    }
